...
\begin{table}[H]
\centering\small
\caption{Headline results overview: model/task, sample size, validation protocol, evaluation metric, and value.}
\label{tab:headline}
\renewcommand{\arraystretch}{1.25}
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{5.1cm}r>{\raggedright\arraybackslash}p{3.3cm}>{\raggedright\arraybackslash}p{2.6cm}r@{}}
\toprule
Model & $N$ & Validation method & Evaluation metric & Value (\%) \\
\midrule
...
\vspace{2pt}
\noindent{\footnotesize LOGO-cluster CV: 500\,m haversine-clustered leave-one-group-out cross-validation.}
\end{table}

\subsection{Eddakhla-Oued Eddahab: the strongest result}
\label{sec:eddakhla}
The strongest single result in this study is a binary Difficult-vs-not classifier evaluated by leave-region-out cross-validation on Eddakhla-Oued Eddahab: \textbf{96.0\% accuracy} against a majority-class baseline of 80.0\% for the same test set (Figure~\ref{fig:eddakhla-hero}; +16 percentage points, $n=25$; identical across three independent training runs).
...
At national scale (all eleven regions pooled, $n=733$), the two binary classifiers reach 74.1\% (Difficult) and 73.3\% (Easy) accuracy (Figure~\ref{fig:map-national}, Table~\ref{tab:confusion}). In an operational deployment, the two are used together: a site flagged neither Difficult nor Easy is assigned Moderate by exclusion, giving a practical three-way screen without requiring a harder direct three-class classification to itself beat either binary result.
