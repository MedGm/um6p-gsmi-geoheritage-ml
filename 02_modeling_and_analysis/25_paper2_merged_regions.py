"""
data_audit/25_paper2_merged_regions.py  (2026-08-23)

Region-merge plan (saved 2026-08-22, queued until after national model +
Paper 1 draft -- both now done). Three regions were too thin for their own
leave-region-out/regional model: Guelmim-Oued Noun (N=7), Laâyoune-Sakia El
Hamra (N=15), Casablanca-Settat (N=7). Merging geographic neighbors gives
real trainable groups instead of leaving them out of Paper 2's regional
table entirely. Same G1 + best-feature-set methodology as
data_audit/24_paper2_best_feature_per_region.py (grid_search + 500m
LOGO-cluster CV + threshold tuning, three feature-set variants: Baseline,
+Geological_Domain, +tourism-infra), but for three new merged region groups:

  South_GuelmimLaayoune            Guelmim-Oued Noun + Laâyoune-Sakia El Hamra
  South_GuelmimLaayouneEddakhla    + Eddakhla-Oued Eddahab (triple, per user's
                                    explicit "also test it also both" request)
  RabatCasablanca                  Rabat-Salé-Kénitra + Casablanca-Settat

All three variants are fresh computation here (no prior "Baseline" run to
reuse, unlike script 24's original 7 regions -- these groupings are new).

Output: results/json/training/phase5_paper2_merged_regions_results.json
"""
...
MERGED_GROUPS = {
    "South_GuelmimLaayoune": ["Guelmim-Oued Noun", "Laayoune-Sakia El Hamra"],
    "South_GuelmimLaayouneEddakhla": ["Guelmim-Oued Noun", "Laayoune-Sakia El Hamra", "Eddakhla-Oued Eddahab"],
    "RabatCasablanca": ["Rabat-Salé-Kénitra", "Grand Casablanca-Settat"],
}
for gname, regs in MERGED_GROUPS.items():
    n = merged["Region"].isin(regs).sum()
    log(f"{gname}: {regs} -> N={n}")
...
results = []
for gname, regs in MERGED_GROUPS.items():
    sub = merged[merged["Region"].isin(regs)].reset_index(drop=True)
    ...
    for target in ["Difficult", "Easy"]:
        ...
        log(f"{gname} / {target}: N={n} pos={n_pos} local_maj={maj:.3f}")
        if n_pos < 3 or n_pos > n - 3:
            log("  SKIPPED -- degenerate class balance")
            continue

        Xr_base = sub[FEATURES_BASE].values
        log("  running Baseline variant ...")
        variants = {"Baseline": run_variant(Xr_base, yb, groups)}
        ...
        log(f"  {gname}/{target}: Baseline={variants['Baseline']['best_acc']} "
            f"Domain={variants['Domain']['best_acc']} Infra={variants['Infra']['best_acc']} -> BEST={best_variant}")

        results.append(dict(group=gname, regions=regs, target=target, N=n, n_pos=n_pos,
                             local_majority=round(maj,3), variants=variants, best_variant=best_variant,
                             best_acc=variants[best_variant]["best_acc"]))

out_path = os.path.join(BASE, "results/json/training/phase5_paper2_merged_regions_results.json")
...
log("\nSUMMARY:")
for r in results:
    print(f"  {r['group']:32s} {r['target']:10s} N={r['N']:3d} best={r['best_variant']:9s} "
          f"acc={r['best_acc']:.3f} local_maj={r['local_majority']} gap={100*(r['best_acc']-r['local_majority']):+.1f}pp")
