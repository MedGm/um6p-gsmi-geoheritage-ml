"""
code/32_extract_features_aug16_new_localities.py  (2026-08-17)

Computes features for the "new" (non-duplicate, region-corrected, in-Morocco)
localities from code/31's ingestion of the 2026-08-16 supervisor-provided
source, using the exact same methodology already used for the 324-site
production catalog and the Aug-9 expansion (code/12):
...
Plus, additionally (code/12 did not need this -- only code/13's favorability
retrain did): Geology_Class, Soil_Class sampled from the same
archive/gis_data/physical_task2_corrected/*.tif rasters code/13 uses, so
these new sites can be used to retrain the favorability model, not just carry
accessibility-side terrain features.

Does NOT assign accessibility labels -- same documented reasoning as code/12:
no expert label exists for these sites, and formula-derived/circular labels
are a known, repeatedly-flagged failure mode in this project's history.

Output: data/newdb_v2_aug16/geosites_new_localities_features.csv
"""
import os
import numpy as np
import pandas as pd
...
OUT_CSV = os.path.join(NEWDB, "geosites_new_localities_features.csv")
...
def sample_projected_raster(lat, lon, src, transformer, window_px=1):
...
def main():
    matched = pd.read_csv(os.path.join(NEWDB, "geosites_localities_vs_final_catalog.csv"))
    ...
    print("Sampling Geology_Class / Soil_Class (for favorability retraining)...", flush=True)
    from pyproj import Transformer
    transformer = Transformer.from_crs("EPSG:4326", PROJ_CRS, always_xy=True)
    with rasterio.open(os.path.join(PHYS_DIR, "geology_classes.tif")) as gsrc, \
         rasterio.open(os.path.join(PHYS_DIR, "soil_classes.tif")) as ssrc:
        df["Geology_Class"] = df.apply(lambda r: sample_projected_raster(r["Latitude_WGS84"], r["Longitude_WGS84"], gsrc, transformer), axis=1)
        df["Soil_Class"] = df.apply(lambda r: sample_projected_raster(r["Latitude_WGS84"], r["Longitude_WGS84"], ssrc, transformer), axis=1)
    print(f"  Geology_Class missing: {df['Geology_Class'].isna().sum()}, Soil_Class missing: {df['Soil_Class'].isna().sum()}", flush=True)
    ...
    # build_localities() (code/01) does not carry a Coordinate_Precision column through to
    # its locality-level output (only tracked per-observation) -- not requested here.
    out_cols = ["Locality_ID", "Geosite_Name", "Latitude_WGS84", "Longitude_WGS84", "Region", "Geosite_Type",
                "Elevation_m", "Slope_deg", "Ruggedness", "Dist_to_Highway_m", "LULC_Friction",
                "Dist_to_Settlement_m", "Geology_Class", "Soil_Class"]
