...
catalog spans **1,154 geosites** across all eleven administrative regions, of which
...
| Geosite-location favorability | 1,154 | Spatial block CV | AUC | **0.927** |
...
| [`data/`](data/) | `final/` — the labeled catalog and dataset used throughout (join key: `Locality_ID`). `model_outputs/` — hyperparameter search results, prediction grids, favorability output. `boundaries/` — region GeoJSON boundaries used for map rendering. `newdb_v2/` — the Aug-2026 catalog-expansion ingestion intermediates. |
| [`code/`](code/) | The full numbered pipeline, `01`–`22`, in run order — catalog consolidation through final model battery. See each script's docstring for what it does and what it reads/writes. |
| [`models/final/`](models/final/) | The one model actually loaded by any current script: the geosite-favorability model (AUC 0.927) behind the favorability map. |
| [`models/experimental/`](models/experimental/) *(gitignored)* | 20 earlier model iterations, kept locally for provenance, not tracked in git. |
...
Work developed by [Mohamed El Gorrim](https://github.com/MedGm) under the supervision of Dr. Ismail Ben Ammar , Mohamed El Ouali and Sanae El Harche at the Geology and Sustainable Mining Institute (GSMI), UM6P, as part of the UM6P AI/ML internship program.